 ...
 /// =============================================================update fcm token 
       ...
       updateFCMToken(){
        console.log('==== i am here ')
                       firebase.FCM.onTokenRefresh().then((data) => {
                            console.log('asdsad', data);
                        }).catch(error => {
                            console.log('UNREGISTER ERRROR', error);
                        })
                    }
//====================================================================== update fcm token
    ...
    logOut = async () => {
        ...
        this.updateFCMToken()
      console.log('log  out token ====>>',updateFCMToken())
    }
    ...
    render() {
        return (
            <View style={{ flex: 1,  }}>
                <AppHeader title="..............." navigation={this.props.navigation} onPress={() => this.logOut()} />
                <FlatList
                    ...
                    ListHeaderComponent={
                        <View style={{ height: 160, justifyContent: 'center', alignItems: 'center' }}>
                            <TouchableOpacity style={{ height: 50, width: 50, borderRadius: 5,borderColor:"red" }} onPress={() => { this.openGallery() }}>
                            ...
                            <Text style={{ color: 'white', fontSize: 25, marginTop: 45, fontWeight: 'bold',marginLeft:25 }}>{this.state.loggedInUserName}</Text>
</View>
    }
                    renderItem={({ item }) => (
                        <View>
                            <TouchableOpacity style={{ flexDirection: 'row', marginBottom: 20, marginTop: 20 }} onPress={() => this.props.navigation.navigate('Chat', { UserName: item.userName, guestUid: item.uuid,token:item.token})}>
                                ...
                                <View style={{ width: '65%', alignItems: 'flex-start', justifyContent: 'center', marginLeft: 10 }}>
                                    <Text style={{ color: 'red', fontSize: 16, fontWeight: 'bold' }}>{item.userName}</Text>
                                    <Text style={{ color: 'red', fontSize: 14, fontWeight: '600' }}>{item.lastMessage}</Text>
                                    <Text style={{ color: 'red', fontSize: 14, fontWeight: '600' }}>temlastMessage</Text>

                                </View>
                                <View style={{ width: '20%', alignItems: 'flex-start', justifyContent: 'center', marginRight: 20 }}>
                                    <Text style={{ color: 'red', fontSize: 13, fontWeight: '400' }}>{item.lastTime}</Text>
                                </View>
                            </TouchableOpacity>
