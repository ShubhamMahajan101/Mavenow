...
import { View, StatusBar, TextInput, Text, StyleSheet, Dimensions, TouchableOpacity, Image, Modal, Alert, ScrollView,BackHandler } from 'react-native'
...
// ************************
  ...
  .then(async (res) => {
  console.log('----->login  screen data',res.user.uid);
  const uid = res.user.uid;
  UpdateOnlineStatus(uid,true)

  var checkTocken = await AsyncStorage.getItem('fcmToken')
  ...
  return (
         ...
         <Image style={styles.mavenowLogo} resizeMode='contain' source={require("./Icon/new_logo_blue_mavenow.png")}></Image>
          ...
          {/* ======================================= Login Button */}
         <TouchableOpacity activeOpacity={0.8} onPress={() => { _loginBtn(),LogintoFirebase() }} style={styles.LoginView}>
